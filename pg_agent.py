...
class PGAgent(nn.Module):
    ...
    def _discounted_return(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        discount_returns = np.full((len_rewards, ), (np.sum(discount_factors * rewards)))
    ...
    def _discounted_reward_to_go(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        """
        discount_reward_to_go = 0.0
        discount_rewards_to_go = [0.0 for i in range(len(rewards))]
        for i in range(len(rewards) - 1, -1, -1):
            discount_reward_to_go = self.gamma * discount_reward_to_go + rewards[i]
            discount_rewards_to_go[i] = discount_reward_to_go
