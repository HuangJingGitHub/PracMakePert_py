...
depth = 0.5
alpha = -1
pypr_expd = np.array([[1, 0, 0], [0, 1, 0], [-0.5, alpha, 0]])    # partial y partial r expanded
...
if __name__ == '__main__':
    ...
    initial_joint_config = np.array([ 0.98579141,  0.4165256 ,  0.17731692, -1.37033894, -0.01099423, -0.5889978 ])
    ...
    while (abs(rotate_angle - 30) > 1): # or centroid_error_norm > 20):
        x_init = psm.get_current_position()
        x_init_position = x_init.p
        x_init_position_np = np.array([[x_init_position.x(), x_init_position.y(), x_init_position.z()]]).T
